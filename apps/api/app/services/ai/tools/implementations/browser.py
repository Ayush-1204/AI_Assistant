...
class BrowserAXTreeTool(BaseTool):
    name = "browser_axtree"
    description = "Navigate a headless browser to a URL and return its Accessibility Tree (AXTree). Use this instead of web_search when you need to understand the visual layout, buttons, and input fields of a specific GUI page natively."
    parameters_schema = {
        "type": "object",
        "properties": {
            "url": {
    ...
    async def execute(self, execution_context: dict, **kwargs) -> str:
        ...
        if ssrf_error:
            logger.warning(f"SSRF Blocked Browser Attempt: {url} | Reason: {ssrf_error}")
