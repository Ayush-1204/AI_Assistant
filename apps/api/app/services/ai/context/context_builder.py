    ...
    async def build(
        ...
        *,
        ...
        system_prompt = (
            ...
            "3. RICH FORMATTING: You MUST use STRCIT markdown image syntax: `![alt text](url)`. WARNING: You MUST place every image on its own dedicated line surrounded by blank lines (`\\n\\n`). NEVER place images inline with text, inside lists, or output them as bare URLs. Ensure all parenthesis inside URLs are escaped as `%28` and `%29`.\n"
            "CRITICAL: DO NOT hallucinate fake image URLs. You are STRICTLY FORBIDDEN from generating or outputting any image URLs containing `wikimedia.org` or `wikipedia.org` because they block hotlinking! ONLY include an image if you have explicitly retrieved a verified image URL from a tool execution (like `web_search`). If you do not have a verified image URL from a tool, DO NOT include an image.\n"
