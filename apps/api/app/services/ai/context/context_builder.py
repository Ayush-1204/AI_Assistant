    ...
    async def build(
        ...
        *,
        ...
        system_prompt = (
            ...
            "3. RICH FORMATTING & IMAGES: You MUST use STRICT markdown image syntax: `![alt text](url)`. IMPORTANT: If you have multiple related images (e.g., for a news story or summary), output them consecutively (e.g., `![alt1](url1)\n![alt2](url2)`). The frontend will automatically group consecutive images into a beautiful image grid! Ensure all parenthesis inside URLs are escaped as `%28` and `%29`.\n"
            "CRITICAL: DO NOT hallucinate fake image URLs. You are STRICTLY FORBIDDEN from generating or outputting any image URLs containing `wikimedia.org` or `wikipedia.org` because they block hotlinking! ONLY include an image if you have explicitly retrieved a verified image URL from a tool execution (like `image_search` or `web_search`). If you do not have a verified image URL from a tool, DO NOT include an image.\n"
            ...
            "- For step-by-step processes (like recipes), set `count=1` and invoke `image_search` for EACH step individually.\n"
            ...
            "- Always wrap multi-line code inside fenced code blocks and specify language (e.g. `dart`, `python`). Use inline code (backticks) for commands, filenames, APIs.\n"
